...
const requests = {
  ...
  post: (url: string, body: any = {}) => api.post(url, body).then(res => res.data),
  put: (url: string, body: any = {}) => api.put(url, body).then(res => res.data),
  patch: (url: string, body: any = {}) => api.patch(url, body).then(res => res.data),
  del: (url: string) => api.delete(url).then(res => res.data),
};

const Expenses = {
  getByUser: (personId: number) => requests.get(`Expenses/user/${personId}`),
  getSummary: (personId: number) => requests.get(`Expenses/user/${personId}/summary`),
  create: (data: any) => requests.post(`Expenses`, data),
  delete: (id: number, personId: number) => requests.del(`Expenses/${id}/user/${personId}`),
};
